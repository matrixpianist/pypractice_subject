...
def generator(data):
    ...
    name=input('my name:')
    #如果路径存在则删除路径
    path = os."F:/pypractice/人脸识别/my_face".join(data,name)
    if os."F:/pypractice/人脸识别/my_face".isdir("F:/pypractice/人脸识别/my_face"):
        shutil.rmtree("F:/pypractice/人脸识别/my_face")
    #创建文件夹
    os.mkdir("F:/pypractice/人脸识别/my_face")
    #创建一个级联分类器
    face_casecade=cv2.CascadeClassifier("F:/opencv/opencv/sources/data/haarcascades/haarcascade_frontalface_default.xml")
    #打开摄像头
    ...
    while(True):
        ...
        if ret:
            ...
            for (x,y,w,h) in face:
                ...
                #保存人脸
                cv2.imwrite('%s/%s.png'%("F:/pypractice/人脸识别/my_face",str(count)),new_frame)
                count+=1
...
def LoadImages(data):
    ...
    for subdir in os.listdir(data):
        subpath=os."F:/pypractice/人脸识别/my_face".join(data,subdir)
        #print('path',subpath)
        #判断文件夹是否存在
        if os."F:/pypractice/人脸识别/my_face".isdir(subpath):
            #在每一个文件夹中存放着一个人的许多照片
            names.append(subdir)
            #遍历文件夹中的图片文件
            for filename in os.listdir(subpath):
                imgpath=os."F:/pypractice/人脸识别/my_face".join(subpath,filename)
