...
rock = pygame.mixer.Sound("F:/音频素材/rock.wav")
...
while keep_going:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pygame.mouse.get_pressed()[0]:
                ...
                if len(clicked_smileys) > 0:
                    rock.play()
                    count_rocked += len(clicked_smileys)
    ...
    draw_string = " - Bubbles rocked: " + str(count_rocked)
    if (count_smileys > 0):
        draw_string += " - Percent: "
        draw_string += str(round(count_rocked/count_smileys*100, 1))
        draw_string += "%"
