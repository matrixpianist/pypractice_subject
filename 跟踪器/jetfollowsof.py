...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
    clock.tick(300)
    x,y = pygame.mouse.get_pos()  #获取鼠标位置，鼠标为打击目标
    distance = sqrt(pow(x1 - x2, 2) + pow(y1 - y, 2))  #两点距离公式
    ...
    missled = pygame.transform.rotate(missile, -(fangle))
    if 0 <= -fangle <= 90:
        A = (width*cosa + x1 - width, y1 - height/2)
        B = (A[0] + height*sian, A[1] + height*cosa)
    if 90< - fangle <=180:
    ...
    if -90 <= fangle <0:
        ...
        B = (A[0] + height*sian, y1 - height/2 + missiled.get_height())
    if -180 < -fangle < -90:
        A = (x1 - width - height*sina, y1 - height/2 + missiled.get_height())
        B = (x1 - width, A[1] + height*cosa)
    C = ((A[0] + B[0])/2, (A[1] + B[1])/2)
    screen.fill((0,0,0))
    screen.bilt(missiled, (x1 - width +(x1 - C[0]), y1 - height/2 +(y1 - C[1])))
    screen.bilt(textc, (x,y))  #鼠标用红色*代替
